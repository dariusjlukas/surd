...
export type CellKind = 'math' | 'markdown'
 ...
 * math statement, in document order. */
export function transcriptOf(cells: Cell[]): string[] {
  return cells
    .filter((c) => c.kind === 'math' && c.status === 'done' && c.result?.ok)
    .map((c) => c.src)
}
...
export const useNotebook = create<NotebookState>()(
  persist(
    (set, get) => {
      ...
      const restartEngine = (transcript: string[]) => {
      ...
      const recomputeFrom = async (notebookId: string, index: number) => {
        ...
        for (const cellId of tailIds) {
          ...
          try {
            const result = await client.eval(cell.src)
            patchCell(notebookId, cellId, { status: 'done', result })
          } catch (e) {
            if (!(e instanceof EngineCancelled)) throw e
      ...
      return {
        ...
        async rerun(cellId: string) {
          ...
          if (index < 0 || nb.cells[index].kind !== 'math') return
